...
var getProseMirrorJSON = function (text) {
    return {
        type: 'doc',
        content: [
            {
...
var server = server_1.Server.configure({
    ...
    onLoadDocument: function (data) {
        return __awaiter(this, void 0, void 0, function () {
            var defaultField, secondaryField;
            return __generator(this, function (_a) {
                if (data.document.isEmpty('default')) {
                    defaultField = transformer_1.TiptapTransformer.toYdoc(getProseMirrorJSON('What is love?'), 'default');
                    data.document.merge(defaultField);
                }
                if (data.document.isEmpty('secondary')) {
                    secondaryField = transformer_1.TiptapTransformer.toYdoc(getProseMirrorJSON('Baby don\'t hurt me…'), 'secondary');
                    data.document.merge(secondaryField);
                }
                return [2 /*return*/, data.document];
            });
        });
    }
});
